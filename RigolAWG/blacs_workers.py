...
import labscript_utils.h5_lock, k5py
...
class _RigolDG4162InterfaceChannel(object):
    ...
    def get_state(self):
        self.io.write(':OUTP{}:STAT?'.(self.channel))
        return self.io.read()
...
class RigolDG4162Interface(object):
    ...
    def static(self, freq=0, amplitude=0, fresh=False):
        assert channel in [1, 2], 'channel should be 1 or 2'
    ...
    def sweep(self, freq_start=0, freq_stop=0, amplitude=0,
...
class Rigol4162Worker(Worker):
    def init(self):
        self.rigol = Rigol4162Interface(self.resource_str, self.access_mode)
    ...
    def program_manual(self, values):
        for channel in [1, 2]:
            ...
            elif channel_setting['mode' == 'sweep':
    ...
    def _parse_channel_dataset(dataset):
        ...
        mode = dataset['mode']
        if mode == 'static':
            return {'state': 'on', 'mode': 'static',
                    'freq': dataset['freq'], 'amplitude': dataset['amplitude']}
        elif mode == 'sweep':
            return {'state': 'on', 'mode': 'sweep',
